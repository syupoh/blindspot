...
from utils import draw_graph, plot_confusion_matrix, draw_fold_graph
from models import compile_model, select_model, adaptive_clip_grad
...
def argparser():
    ...
    parser.add_argument('--gpu', '-g', type=str, help='select gpu to train', default=7)
...
def fit(model, args, train_dataset, val_dataset, save_path, best_acc=-1):
    ...
    @tf.function
    def train_step_agc(batch_x, batch_y):
        with tf.GradientTape() as tape:
            predictions = model(batch_x, training=True)
            loss = loss_function(batch_y, predictions)
        ...
        optimizer.apply_gradients(zip(agc_gradients, trainable_params))
        train_loss(loss)
        train_acc(batch_y, predictions)
    ...
    #     gradients = tape.gradient(loss, model.trainable_weights)
    #     optimizer.first_step(gradients, model.trainable_weights)

    #     with tf.GradientTape() as tape:
    #         predictions = model(images, training=True)
    #         loss = loss_function(labels, predictions)
    #     gradients = tape.gradient(loss, model.trainable_weights)
    #     optimizer.second_step(gradients, model.trainable_weights)
    #     train_loss(loss)
    ...
    save_dataset(train_dataset, os.path.join(save_dir, f"fold_{n_fold}_train.txt"))
    save_dataset(val_dataset, os.path.join(save_dir, f"fold_{n_fold}_val.txt"))
    ...
    for epoch in t:
        ...
        for i, (batch_x, batch_y) in enumerate(train_dataset):
            ...
            # train_step_agc(batch_x, batch_y)
            # train_step_SAM(batch_x, batch_y)
...
def main():
    ...
    if args.cross_validation:
        ...
        for train_idx, test_idx in kfold.split(X, y):
            ...
            train_dataset = EGDSpotDataset.initialize_from_data(X_train.tolist(), y_train.tolist(), args.batch, image_shape, scale=args.scale, train=True)    
            test_dataset = EGDSpotDataset.initialize_from_data(X_test, y_test, args.batch, image_shape,  scale=args.scale, train=False)    
        ...
        if args.val_path == "":
            ...
            train_dataset = EGDSpotDataset.initialize_from_data(X.tolist(), y.tolist(), args.batch, image_shape, scale=args.scale, train=True)    
            test_dataset = EGDSpotDataset.initialize_from_data(X_test.tolist(), y_test.tolist(), args.batch, image_shape, scale=args.scale, train=False)    
            train.append(train_dataset)
            test.append(test_dataset)

    save_path = f"/data/syupoh/dataset/endoscopy/JFD-03K/ckpt/{args.ckpt_name}/{datetime.today().strftime('%Y%m%d')}.h5"
    save_dir = os.path.dirname(save_path)
